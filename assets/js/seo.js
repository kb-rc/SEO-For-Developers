...
(function () {
  ...
  function buildToC() {
    ...
    tocEl.innerHTML = Array.from(headings).map(h =>
      `<a class="toc-link sub" href="#${h.id}" data-anchor="${h.id}">${h.textContent.trim()}</a>`
    ).join('');
  }
    ...
    const observer = new IntersectionObserver(entries => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          const link = document.querySelector(`.toc-link[data-anchor="${entry.target.id}"]`);
  ...
  function setPageTitle(primary, brand) {
    if (brand === undefined) brand = 'SEO Reference';
   ...
   */
  var REDIRECTS = {
    '/SEO-For-Developers/blog/json-ld-guide.html': '/SEO-For-Developers/docs/json-ld.html',
    '/SEO-For-Developers/docs/structured_data':    '/SEO-For-Developers/docs/json-ld.html'
  };
